...
#for(net_index in 1:1){  
  ...
  for(module_index in 1:length(network_genesets)){
    ...
    if(length(module_common_genes)>1){
      ...
      cvfit=glmnet(gene_pathway_matrix,module_labels,lambda = 0.007956622,alpha = alpha)   #s5;0.007956622, dream[[1]]:
  ...
  write.csv(dream_consensus_modules_results,
            file = paste(result_path,'DanielsPaper/','dream_consensus_modules_results_',unique_network_names[[net_index]],'_alpha',alpha,".csv",sep = ""),
            row.names=FALSE, col.names = TRUE)

}


####################### For modules from top5 methods

module_genesets=scan("dream_top_methods_modules.gmt",what = "",sep = "\n")
module_genesets=strsplit(module_genesets,split = "\t")
...
for(net_index in 1:length(unique_network_names)){
  ...
  write.csv(dream_consensus_modules_results,
            file = paste(result_path,'DanielsPaper/','dream_top_methods_modules_results_',unique_network_names[[net_index]],'_alpha',alpha,".csv",sep = ""),
            row.names=FALSE, col.names = TRUE)
